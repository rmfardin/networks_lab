...

{
    ...
    for (i = 0; i < 4; i++) initrtpkt->mincost[i] = mincosts[i];
}
...

{
    ...
    if (packet.sourceid == packet.destid) {
        printf(
            "WARNING: source and destination id's the same, ignoring "
            "packet!\n");
        return;
    }
    if (connectcosts[packet.sourceid][packet.destid] == 999) {
        printf(
            "WARNING: source and destination not connected, ignoring "
            "packet!\n");
        return;
    }
    ...
    for (i = 0; i < 4; i++) mypktptr->mincost[i] = packet.mincost[i];
    if (TRACE > 2) {
